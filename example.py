...
if "curr_state" not in st.session_state:
    ...
    edges = [
        StreamlitFlowEdge("1-2", "1", "2", animated=True, marker_start={}, marker_end={"type": "arrow"}),
    ...
    if st.button("Delete Random Node"):
        if len(st.session_state.curr_state.nodes) > 0:
            ...
            st.session_state.curr_state.nodes = [
                node for node in st.session_state.curr_state.nodes if node.id != node_to_delete.id
            ]
...
with col5:
    if st.button("Random Flow"):
        nodes = [
            StreamlitFlowNode(
                str(f"st-flow-node_{uuid4()}"), (0, 0), {"content": f"Node {i}"}, "default", "right", "left"
            )
            for i in range(5)
        ]
        edges = []
